...
nix --extra-experimental-features 'nix-command flakes' \
  profile add github:nuggocto/suzumushi/v1.1.6
```
...
[`v1.1.6` release](https://github.com/nuggocto/suzumushi/releases/tag/v1.1.6),
either in your browser or with:

```sh
curl -fLO \
  'https://github.com/nuggocto/suzumushi/releases/download/v1.1.6/{SHA256SUMS,suzumushi-v1.1.6-x86_64-unknown-linux-gnu.tar.xz}'
```
...
tar -xf suzumushi-v1.1.6-x86_64-unknown-linux-gnu.tar.xz
cd suzumushi-v1.1.6-x86_64-unknown-linux-gnu
...
Unchanged files reuse metadata saved during the previous terminal startup.
The disposable cache at `state/metadata.jsonl` holds at most 4 MiB and 10,000
entries. Delete it while the player is closed to force a fresh metadata read.
`diagnose` can read the cache but never writes it.
...
Version `1.1.6` is the current stable release. Its tag-driven Linux archive is
