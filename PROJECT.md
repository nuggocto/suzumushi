...
tags: [rust, project, linux, tui, local-audio, music, podcasts, audio-drama, ratatui, mpris, waybar, tmux, zellij, visualizer]
created: 2026-07-04
updated: 2026-08-10
status: planning
---
...
17. [Tag Model](#tag-model)
18. [Artwork Model](#artwork-model)
19. [MPRIS Model](#mpris-model)
20. [Waybar Model](#waybar-model)
21. [Tmux Model](#tmux-model)
22. [Zellij Model](#zellij-model)
23. [UI Direction](#ui-direction)
24. [Phases](#phases)
25. [Testing Strategy](#testing-strategy)
26. [Safety And Security Notes](#safety-and-security-notes)
27. [Resource Bounds](#resource-bounds)
28. [Release And Packaging](#release-and-packaging)
29. [Final Repo Structure](#final-repo-structure)
30. [Future Ideas After V1](#future-ideas-after-v1)
...
- Sits on a windowsill or a curled leaf under a low warm lamp.
...
-> the Art panel shows album details, playback progress, and a small visualizer
...
terminal, or minimal session. The core TUI, filesystem, queue, playback,
...
- Artwork region inside the persistent TUI Art panel through terminal image protocols when supported
- MPRIS `mpris:artUrl`
...
`verify`, `restore`, and explicit `prune` commands before tag editing ships. A
...
Do not use live inotify watching in v1. It is useful later, but MPRIS, tag editing, and audio controls are already enough for the first release.
...
`Space` doubles as a leader key for chords like `Space+/`. When
...
wins. Chords require an unmodified second key. An invalid second key cancels the
pending leader when it has its own action; otherwise the pending play/pause
fires. Either path emits at most one action for that input event. This is the
only timing-based disambiguation rule. Every other
...
```txt
A          toggle cover artwork inside the Art panel
```
...
| Logging | `tracing` + `tracing-subscriber` feeding an app-owned bounded worker and rotating writer | File logging without corrupting the TUI; the application owns queue admission, drop counting, byte rotation, retained-file limits, failure reporting, and joined shutdown |
| Unicode display | `unicode-width` | Correct width for titles and artists |
...
| Foundation/errors/logging | `thiserror = 2.0.20`, `tracing = 0.1.44`, `tracing-subscriber = 0.3.23`; `anyhow` remains unselected | Tracing uses only `std`; subscriber uses `fmt` and `std`; the standard-library bounded channel and joined worker keep queue and lifecycle ownership inside the application |
| TUI/runtime | `ratatui = 0.30.2`, `crossterm = 0.29.0`; `tokio` and `unicode-width` remain unselected direct dependencies | Ratatui selects only `crossterm_0_29`; the direct Crossterm edge disables defaults and selects `events`, while Ratatui's adapter transitively unifies Crossterm's default feature set; the resolved permissive licenses add Zlib through `foldhash`; an app-owned synchronous loop is sufficient until a real worker needs async runtime behavior |
...
| Current tests/fuzz | `tempfile = 3.27.0`, `insta = 1.48.0`, `cargo-fuzz = 0.13.2`, `libfuzzer-sys = 0.4.13` | Temporary-root and reviewed terminal snapshots plus five bounded untrusted-input targets; later property/CLI helpers remain unselected |
...
| model/                - tracks, playlists, queue, and tags          |
...
- Logs always go to files, never stdout while the TUI is active. An app-owned bounded queue drops and counts diagnostic records instead of blocking playback, while its single owned worker performs byte/count rotation. Shutdown closes admission, drains the queue, joins the worker without detaching it, and only then reports queue drops and background write/flush failures after terminal restoration.
...
AppAction      -> UI queue mutation or AudioCommand / ScanRequest / SearchQuery / TagRequest / ArtworkRequest
...
read-only media work carry `scan_generation`. Stale scan, artwork lookup, state
projection, audio-position, and visualizer events are discarded. Committed
tag/restore mutation acknowledgments are never discarded by a
...
expectations and never substitutes for a critical release invariant. Untrusted media and tag input must never reach
...
- Scanner descent below `audio/` is limited to `library/` and `playlists/`; unrelated top-level entries remain untouched and their descendants do not consume scan budgets
...
- External symlink targets are playback/read-only in v1. Tag, restore, backup, cache, state, log, and journal mutation targets must resolve beneath the canonical root; an external target is refused before backup or write.
- Symlinked directories are ignored in v1
- Canonical paths are used for deduplication
- Canonical paths deduplicate media assets only. Playlist entries remain distinct so one asset can appear in the library and multiple playlists with different names and order.
- Display paths should be relative to the Suzumushi root when possible
- `init` canonicalizes the root, refuses to overwrite non-Suzumushi/non-empty destinations without explicit confirmation, does not follow a symlinked root silently, and acquires the descriptor-relative root writer lease before it creates or repairs owned entries
...
manifests, tag replacements, and recovery state retain the full sync
contract.
...
- Editing has no backup bypass: every tag write requires its configured verified backup. If backup preflight or creation fails, editing fails closed.
...
- Queue, playlist, metadata, path, log, state, channel, PCM, artwork, tag, backup, and shutdown limits are mandatory from their first phase
...
- Logging uses both count and byte bounds plus app-owned size/count rotation. Formatting rejects or visibly truncates a diagnostic above `max_record_bytes` before enqueue; the worst-case 8 MiB queue capacity is reserved from the UI/state/logging sub-budget and process ledger up front. Lowering `max_files` prunes archives outside the new bound during startup. The default queue-full policy drops and counts diagnostic records rather than blocking the UI/audio path; after the TUI restores the terminal, shutdown closes admission, drains the queue, joins the worker, and writes the final dropped count directly to diagnostics. Background write, flush, rotation, and worker failures prevent a false successful exit.
- The current startup preflight reserves both 112 MiB indexes, 32 MiB of scanner/parser scratch, the configured logging byte queue, and 8 MiB of UI/state scratch. It also rejects a log record larger than one retained file and any count-based queue shape that exceeds its byte reservation. Initial and resized terminal dimensions are checked against that 8 MiB reservation before Ratatui allocates two cell buffers; resize replaces the old fixed viewport only after validation so allocator growth cannot exceed the reservation through hidden autoresize.
- `max_open_files`, `max_blocking_jobs`, `max_parser_helpers`, and worker-thread counts are hard ownership limits enforced before open/spawn. Mutable TUI startup reserves its exact five-descriptor peak before acquiring either lease or opening logging storage. Blocking parser, artwork, backup, and mutation work uses bounded semaphores; diagnostics expose current/high-water counts, and limit-plus-one tests prove refusal without leaked descriptors or detached tasks.
...
- A `MediaAsset` is deduplicated by canonical path plus verified file identity; a `TrackEntry` preserves library/playlist membership and display context.
...
- Perform exactly one descriptor-rooted traversal of `suzumushi/audio/` per scan request using the app-owned `rustix` walker; descend only into library and playlists, and classify library, immediate playlist children, and artwork candidates during that traversal rather than walking any subtree again
...
Every audio, tag, image, state, config, journal, and
...
- Playback position must remain accurate enough for progress, MPRIS, Waybar, tmux, Zellij, and notifications
...
- `source_position` is the media timeline consumed by progress, MPRIS, and status outputs. At 2.0x it advances about two media seconds per wall second; at 0.5x it advances about half a media second.
...
The visualizer should feel like the song is gently breathing in the terminal. It should not compete with album art.

---

## Mutation And Backup Model

Before tag editing is enabled, Suzumushi provides
...
editing adds its format-specific payload and post-write validation.
...
`journal_max_total_bytes`, tag backup entry limits, and
...
- Store a bounded versioned manifest for each actual mutation variant: full-file tag backup and destructive restore preservation
...
- Keep the Art panel's album, playback state, progress, elapsed/duration, and visualizer independent from its optional image region; artwork failure or toggling never removes the timeline
...
- Update on play/pause/stop, volume, mute, speed, seek acknowledgment, device recovery, and artwork-cache completion
...
|                      | Yume Utsutsu               | 01 Yume Utsutsu      |
| Library              | Lamp                       | 02 Rain Song         |
| Playlists            |                            | 03 Quiet Morning     |
|                      +----------------------------+                      |
|                      |            Art             |                      |
|   focus              | [ cover ] Tokyo Utopia     |                      |
|   night-drive        | [ image ] PLAYING  1.0x    |                      |
|   sleep              | [=======>------]           |                      |
|                      | 01:42 / 04:02  vol 70%     |                      |
|                      | (( ▁▂▄▆█▅▃▂ ))             |                      |
+----------------------+----------------------------+----------------------+
| NORMAL  Space play/pause  n next  p prev  Space+/ palette  A cover  ? help |
+----------------------------------------------------------------------------+
```

Panel titles stay centered and keep the same label when focus changes. Double
borders and text emphasis show focus without moving the title or relying on color.

The shallow Now Playing panel is a compact identity strip. It shows the current
item title and, when a second line fits, its primary creator. It does not own the
progress bar, timer, playback controls, or visualizer.

The larger Art panel is the persistent current-track canvas. It keeps the album
or series name, playback state, progress bar, `elapsed / duration`, volume and
speed summary, and mini visualizer together with the cover image. The progress
and timer use the audio path's acknowledged source position, freeze while paused,
follow seek and speed changes, and clamp to known duration. They are never guessed
from an independent UI timer. When artwork is missing, unsupported, or toggled
off, only the image region becomes a calm text fallback; the track details and
playback timeline remain visible.
...
The mini visualizer belongs in the Art panel beneath the playback timeline. It
should use soft bars, Suzu wing/ring motifs, or a mono-safe fallback. It should
disappear or become a tiny idle motif in small terminals before pushing out the
title, playback state, progress, elapsed/duration, queue, or status controls.
...
Each phase should end with `cargo run -- --root ./suzumushi` or a small CLI command showing something real. Avoid a big-bang build where playback, MPRIS, notifications, artwork, and tag writes all land at once.
...
- [x] **Phase 3** - Minimal TUI Shell - *milestone: Terminal prototype* - *tag 0.2.0*
...
- [ ] **Phase 7** - Album Art Pipeline And Terminal Display - *milestone: Artwork MVP* - *tag 0.4.0*
- [ ] **Phase 8** - Waybar, Tmux, And Zellij Status Outputs - *milestone: Status outputs MVP* - *tag 0.5.0*
- [ ] **Phase 9** - MPRIS, Media Keys, Notifications, And MPRIS Album Art - *milestone: Desktop MVP* - *tag 0.6.0*
- [ ] **Phase 10** - Safe Single-Track And Batch Tag Editing - *milestone: Safe metadata MVP* - *tag 0.7.0*
- [ ] **Phase 11** - Search Palette Polish, Diagnostics, And V1 Fit-And-Finish - *milestone: V1 fit-and-finish* - *tag 0.8.0*
- [ ] **Phase 12** - Linux Release Candidate - *milestone: Release candidate* - *tags 1.0.0-rc.N then 1.0.0*
...
| Artwork MVP | Phase 7 | Album art is extracted, cached, and displayed in supported terminals |
| Status outputs MVP | Phase 8 | Waybar, tmux, and Zellij commands expose now-playing state |
| Desktop MVP | Phase 9 | MPRIS/media keys, MPRIS album art, and track-change notifications work |
| Safe metadata MVP | Phase 10 | Single-track and batch tag edits work with backups and confirmations |
| V1 fit-and-finish | Phase 11 | Global palette, diagnostics, terminal-native theme, and empty/error states are release-grade |
| Release candidate | Phase 12 | Docs, tests, packaging, and Linux QA are clean |
...
| 0.4.0 | Phase 7 | Artwork MVP |
| 0.5.0 | Phase 8 | Status outputs MVP |
| 0.6.0 | Phase 9 | Desktop MVP |
| 0.7.0 | Phase 10 | Safe metadata MVP |
| 0.8.0 | Phase 11 | V1 fit-and-finish |
| 1.0.0-rc.N | Phase 12 | Release candidate |
| 1.0.0 | Phase 12 gates pass | First advertised Linux release |
...
**Expected result:** `suzumushi init ./suzumushi` creates the directory structure, and `suzumushi diagnose --root ./suzumushi` reports discovered tracks, artwork candidates, and warnings.
...
2. [x] Implement `suzumushi init <path>` with canonical-root validation and fail-closed behavior for symlinked or non-empty unrelated destinations. Acquire and verify the descriptor-relative root writer lease before creating or repairing owned entries. Create app-private `state/`, `backups/`, and `logs/` as `0700`, private files as `0600`, and verify existing ownership/modes before use.
3. [x] Write default `config.toml`.
4. [x] Create `audio/playlists/demo/README.txt` with empty placeholder instructions.
5. [x] Create `paths.rs` with root, audio, library, playlists, state, artwork cache, logs, and backups paths.
6. [x] Implement supported audio extension detection.
7. [x] Implement one app-owned descriptor-rooted `scan/` traversal of `audio/` using `rustix` directory descriptors and the documented `openat2`/component-wise secure-open policy; do not invoke a pathname walker or a second traversal for library, playlists, or artwork.
8. [x] Classify `audio/library/` recursively without assigning special playback semantics to user folder names.
9. [x] Classify child folders of `audio/playlists/` as playlists during the same traversal.
10. [x] Treat file symlinks as non-descending entries and support them only through the bounded secure-open adapter after resolving/opening a verified regular target from the pinned parent. External targets are read/play-only; an unsupported kernel/filesystem security primitive returns `unsupported_secure_open` with no path-based fallback.
11. [x] Ignore directory symlinks and record every mount-boundary decision under configured counters.
12. [x] Detect sidecar artwork candidates such as `cover.jpg`, `cover.png`, `folder.jpg`, and `folder.png`.
13. [x] Read lightweight metadata needed for search, including artist, album artist, album, and title.
14. [x] Emit filename/path fallback search fields when metadata is missing.
15. [x] Add scan bounds for every encountered entry/directory/symlink/special file, file count, depth, playlists, entries per playlist, symlink resolutions, parser attempts, mount boundaries, per-path and aggregate path bytes, metadata fields/total, warning bytes, and one inclusive complete-index budget reserved alongside the old index under the process-wide budget.
16. [x] Build separate canonical `MediaAsset` and contextual `TrackEntry` models with defined stable-within-path IDs and scan generation.
17. [x] Enforce the exact root precedence `--root`, then `SUZUMUSHI_ROOT`, then existing `./suzumushi`, then error; an invalid selected higher-priority source does not fall through.
18. [x] Implement lock primitives and verified stale-lock handling. The per-user global lock uses only the verified current-UID `$XDG_RUNTIME_DIR/suzumushi/` location with no shared-`/tmp` fallback; the root lock stays descriptor-relative beneath the canonical root. Initialization acquires only the root mutation lease before root writes. Acquire the per-user active-TUI lock and then the canonical-root single-writer lock when mutable TUI mode starts in Phase 3. Document and test that two concurrent login sessions for the same UID intentionally contend. Phase 2 `diagnose` and later status commands remain bounded read-only readers and acquire neither writer lock. Any future standalone mutating command acquires an explicit root mutation lease without claiming the global MPRIS/TUI identity.
19. [x] Before a third-party metadata parser sees media, use the `security` skill to record the concrete parser and filesystem risks in a concise root `SECURITY.md`. Record the selected parser's bounded in-process contract in `docs/parser-isolation.md`; use a helper process only if the chosen API cannot enforce the required input/work bounds. Add dependency checks to `mise.toml` when these runtime dependencies arrive.
20. [x] Return structured scan warnings.
21. [x] Add tests with temporary roots, all root-source/conflict/error combinations, invalid higher-priority roots, duplicate assets across playlists, repeated entries, copied files, symlinked files, retargeted/broken/external symlinks, FIFO media/config/README entries, exact nested initialization error paths, parent-directory rename/substitution during enumeration, magic-link refusal, verified regular-descriptor refusal, no pathname re-open instrumentation, unsupported secure-open behavior, component-wise natural-order fixed vectors, reversible invalid-path-byte display, root-level hidden and unrelated directory budget exclusion, symlinked roots, non-empty init destinations, private-directory ownership/mode refusal, secure runtime-lock location and missing-runtime failure, same-UID concurrent-session lock contention, artwork candidates, searchable metadata, every counter at limit and limit+1, mount boundaries, one-traversal instrumentation, simultaneous old/new index and scanner/parser scratch reservation, and ignored directory symlinks.
22. [x] Add phase-owned fuzz targets for scanner path classification, config, lightweight metadata adapter input, and terminal-safe display projection. PR smoke runs use pinned total-time, per-input timeout, input-length, and RSS bounds; helper-backed parsers also fuzz bounded IPC framing and crash/timeout cleanup.
...
- [x] One canonical asset can retain multiple playlist entries.
...
- [x] Unrelated top-level directories below `audio/` are not traversed, and their descendants cannot consume scan budgets.
...
- [x] Reinitialization verifies and acquires the root writer lease before mutation, refuses an unsafe lock entry, and leaves missing owned entries untouched while another writer holds the lease.
...
1. [x] Add only the exact minimal TUI/runtime/logging dependencies used by this phase.
2. [x] Configure bounded file logging before entering the TUI; never write logs to the active terminal surface, and own an explicitly joined worker so queued writes and failures cannot outlive a successful return.
3. [x] Create `terminal.rs` with raw mode and alternate screen cleanup.
4. [x] Before terminal or MPRIS initialization, reserve the exact five-descriptor startup peak, verify the private current-UID runtime directory, and acquire locks in the documented order: per-user global active-TUI lock, then canonical-root single-writer lock. Refuse a lower `max_open_files`, insecure/missing runtime storage, and a second mutable TUI clearly, including one in another login session for the same UID; verify stale-lock process identity and release both on every normal/error/unwinding-panic path. Document that abort, `SIGKILL`, and power loss cannot run destructors and require stale-lock/terminal recovery on the next start.
5. [x] Query terminal image protocol capability after entering alternate screen with a strict bounded timeout and cancellation path, but do not render art yet. Keep only one reply-producing query outstanding, accept only a complete framed reply, and never hand a partial reply to normal input. Missing replies, pure Wayland, and tmux/Zellij without passthrough select fallback without delaying startup indefinitely.
6. [x] Create `app.rs` with `AppState`.
7. [x] Create `event.rs` for keyboard, resize, tick, and app events.
8. [x] Create `input.rs` for keybindings, including the `Space` leader rule: an exact unmodified chord key within the bounded, configurable `leader_timeout_ms` (default 250 ms) fires the chord; timeout expiry with no second key fires play/pause, and an invalid sequence resolves to no more than one action.
9. [x] Create `ui/layout.rs` and `ui/status.rs`.
10. [x] Draw placeholder panels for Library, Now Playing, Queue, and Art.
11. [x] Add a bottom status/help bar.
12. [x] Support `q` to quit.
13. [x] Support `Tab` and `Shift+Tab` to cycle focus.
14. [x] Support terminal resize without panic; validate checked cell-buffer bytes against the 8 MiB UI/state reservation before initial allocation and every resize, then refuse an oversized terminal before buffer growth.
15. [x] Add small-terminal fallback.
16. [x] Add first `insta` snapshots at 80x24 and 120x32.

#### Done when

- [x] TUI opens in alternate screen.
- [x] Exit restores the terminal.
- [x] Resize works within the allocation reservation, while an oversized resize fails closed and restores the terminal before diagnostics.
- [x] Focus changes are visible.
- [x] Image protocol capability is recorded without breaking terminals that do not answer queries.
- [x] Logs do not corrupt the TUI, and shutdown drains and joins the owned worker before reporting success or failure.
...
13. [ ] Render the current title and primary creator in the compact Now Playing panel. Render album or series, playback state, a progress bar, and `elapsed / duration` in the persistent Art panel from coalesced playback position; pause freezes it and known duration clamps it.
...
- [ ] Now Playing stays a compact track identity while the Art panel keeps playback state, progress, and elapsed/duration visible with or without cover artwork.
...
7. [ ] Implement seek forward/backward and update the Art panel timeline from the acknowledged source position; speed changes advance that same timeline at the media rate.
...
15. [ ] Render the visualizer inside the Art panel beneath the playback timeline, with idle states for pause, stop, and silence.
...
- [ ] The authoritative source clock preserves the selected audio contract at every required speed; later MPRIS and status work validates only consumer projections against it.
- [ ] `1.0x` bypasses the stretcher.
- [ ] Seek handles start/end boundaries.
- [ ] The Art panel progress and elapsed/duration follow pause, seek, and playback speed without an independent timer.
...
### Phase 7 - Album Art Pipeline And Terminal Display
...
**Expected result:** Tracks with embedded or sidecar artwork show art in the persistent Art playback surface on supported terminals and a calm image-region fallback elsewhere, without hiding track details or the timeline.
...
8. [ ] Render artwork in the Art panel's bounded image region with `ratatui-image` when supported, without displacing its track details or playback timeline.
9. [ ] Render a calm image-region text fallback when no protocol or artwork is available while keeping album, state, progress, and elapsed/duration visible.
10. [ ] Add `A` to toggle only the cover artwork region; the Art panel and its playback information remain present.
...
- [ ] TUI renders art or an image-region fallback without corrupting the terminal or hiding current-track playback information.
...
### Phase 8 - Waybar, Tmux, And Zellij Status Outputs
...
11. [ ] Add playing, paused, stopped, muted, and error classes for Waybar, matching the Waybar Model list exactly.
...
### Phase 9 - MPRIS, Media Keys, Notifications, And MPRIS Album Art
...
0. [ ] Verify `souvlaki` exposes every field in the MPRIS Model table (`albumArtist`, `trackNumber`, `genre`) and supports per-generation `SetPosition` filtering; if any is missing, adopt `zbus` directly. Record the decision as an ADR before any other Phase 9 step.
...
11. [ ] Update MPRIS metadata on track change, tag edit, and artwork cache update.
...
### Phase 10 - Safe Single-Track And Batch Tag Editing
...
15. [ ] Implement the journaled backup/list/verify/restore/previewed-prune and recovery foundation before enabling tag writes.
...
### Phase 11 - Search Palette Polish, Diagnostics, And V1 Fit-And-Finish
...
2. [ ] Add command results for rescan, shuffle, repeat, tag editor, batch tag editor, art toggle, visualizer toggle, diagnostics, and quit.
...
15. [ ] Add empty-state screens for no music, no playlists, no art, no visualizer data, no audio device, no D-Bus, no notification daemon, and unsupported image protocol.
...
### Phase 12 - Linux Release Candidate

**Status:** `not started`

**Goal:** Package, document, and verify Suzumushi as a Linux-only local audio player.

**Why this comes last:** Releasing before audio, artwork, MPRIS, notifications, Waybar, tmux, Zellij, and tag editing are stable would create support pain.

**Expected result:** A user can install Suzumushi, create a root, drop music into folders, configure Waybar/tmux/Zellij, use media keys, see notifications, album art, and the mini visualizer, edit tags safely, and understand the limits.

#### Build steps

1. [ ] Write README with install, init, folder layout, controls, artwork, mini visualizer, Waybar, tmux, Zellij, MPRIS, notifications, and tag editing safety.
2. [ ] Add `suzumushi diagnose` output for root/config/audio/DSP/visualizer/artwork/MPRIS/notifications/Waybar/tmux/Zellij checks.
...
- Generic list/verify/previewed-prune and destructive restore behavior protects tag backups
- Full-file tag and restore journal paths reach the correct `Completed`, `RolledBack`, `RecoveryRequired`, or `Unrecoverable` disposition after every injected interruption, malformed/truncated/checksum-invalid journals included
- Journal record bytes, aggregate bytes, entry counts, tag backup entries, and startup recovery attempts pass at limit and refuse limit+1 without deleting existing evidence or starting a mutation
...
- Compact Now Playing identity plus Art-panel playback, progress, and timer snapshots
- Art-panel mini visualizer playing and idle snapshots
...
playback, artwork, status renderers, MPRIS/notifications, tag preview and
...
- Use property tests only where one clear invariant covers meaningful input space better than a small example table, such as natural ordering.
...
pause/resume
seek
Art panel progress and elapsed/duration follow play, pause, seek, and speed
speed 0.5x and 2.0x
...
disk-full and permission-denied tag/state/artwork paths
backup list/verify/restore/previewed-prune
audio output device loss and recovery
shutdown during scan, artwork decode, tag save, and playback; committed mutation retains root lock and recovers from its journal
...
- Disclosure of personal recordings, paths, and tags through backups, journals, logs, or overly permissive app-owned directories
...
- Treat audio files, tags, and filenames as untrusted input
- Treat images and embedded artwork as untrusted input
- Never render raw control characters from tags, filenames, search results, tmux output, or Zellij output
...
- Rotate logs within configured byte/count limits and omit raw tag values, paths outside the root, and state payloads by default
...
- Do not allocate giant strings for tags
...
Phase 12, and `v1.0.0` as the first advertised release. Package and changelog
...
- Create a separate repository named `suzumushi-front` only after Phase 12 has produced stable installation commands and a real changelog.
