...
- A minimal `Cargo.toml`, `Cargo.lock`, and `src/main.rs` exist. `mise.toml`, CI, the library target, and project tests do not exist yet, so do not claim their verification or invent commands before setup adds them.
...
- Initial full verification is `mise run ci`, in this order: `fmt`, `clippy`, `test`, `msrv`. Add security, fuzz, fault, QA, and release tasks only when their corresponding dependency or feature exists.
...
- Mutable TUI mode takes the canonical-root writer lock and then the secure per-UID active-TUI lock under verified `$XDG_RUNTIME_DIR`; status commands remain concurrent. Do not use `/tmp` or blindly remove stale locks.
