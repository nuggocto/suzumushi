...
Existing release tags do not gain the flake retroactively; use `shrek` or a
commit containing it until a new release includes it.
...
```sh
nix profile add github:nuggocto/suzumushi/shrek
```
...
```sh
nix run github:nuggocto/suzumushi/shrek -- init ./suzumushi
nix run github:nuggocto/suzumushi/shrek -- --root ./suzumushi
```
...
```nix
inputs.suzumushi.url = "github:nuggocto/suzumushi/shrek";
```
